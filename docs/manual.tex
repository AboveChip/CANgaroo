...
The trace can also be written to file from a script, using the same writers as
\ui{File} $\rightarrow$ \ui{Save Trace}. The format is taken from the
\file{format} argument, or inferred from the file extension when \file{format}
is omitted.

\begin{lstlisting}[language=Python, caption={Save the trace to a file}]
cangaroo.save_trace("/tmp/run.asc")               # format from extension
cangaroo.save_trace("/tmp/run.pcapng")
cangaroo.save_trace("/tmp/run.bin", format="pcap")  # explicit format
\end{lstlisting}

\begin{tabular}{ll}
  \textbf{Format name} & \textbf{Extensions} \\
  \hline
  \file{candump} & \file{.candump}, \file{.log} \\
  \file{asc}     & \file{.asc} \\
  \file{mdf}     & \file{.mf4}, \file{.mdf}, \file{.mdf4} \\
  \file{pcap}    & \file{.pcap} \\
  \file{pcapng}  & \file{.pcapng} \\
\end{tabular}

\medskip
Unlike the save dialog, which falls back to Vector ASC for an unrecognised
extension, \file{save\_trace} raises \file{ValueError} rather than guessing, and
\file{RuntimeError} if the file cannot be written. Saving is safe while a
measurement is running. See \file{examples/save\_trace.py}.
...
\subsection{LIN Diagnostics}

LIN-capable interfaces expose a diagnostics API for node configuration and
identification (as defined by LIN specification section~4.2).

\begin{description}[leftmargin=2em, style=nextline]
  \item[Diagnostic Request (master)]
    \texttt{sendLinDiagRequest(nad, data, len)} sends a master-request frame
    to the node with the given NAD (Node Address for Diagnostics, 0x01--0x7E).
    The payload follows the LIN diagnostic frame format (up to 6 data bytes).

  \item[Diagnostic Response (slave)]
    \texttt{sendLinDiagResponse(nad, data, len)} sends a slave-response frame
    on interfaces that advertise the \texttt{capability\_lin\_slave} flag.
    Use this to answer a master request when CANgaroo is acting as a LIN slave node.
\end{description}

\begin{notebox}
Both calls are no-ops on interfaces that do not support the corresponding
LIN capability flag. Check the interface capabilities in the Setup Dialog
before relying on diagnostic operations.
\end{notebox}
...
Open \menu{File \textrightarrow{} Settings\ldots} to adjust application-wide
preferences. The dialog is organised into four groups.
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \midrule
    Theme     & Qt style applied to the whole application.
                The available entries depend on the platform
                (e.g.\ \textit{Fusion}, \textit{Windows}, \textit{kvantum}). \\
    Language  & UI language. A restart is required for the change to take full
                effect. \\
    Font size & Application-wide font size in points (range: 6\,--\,24\,pt). \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    Restore window layout on startup
      & When checked, CANgaroo restores the dock layout and window
        geometry from the previous session. \\
    Clear trace on measurement start
      & When checked, the trace buffer is automatically cleared each time
        a new measurement starts. \\
    Do not ask to save workspace on close
      & When checked, CANgaroo closes without prompting to save unsaved
        workspace changes (changes are silently discarded). \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \midrule
    Default view
      & The initial display mode for newly opened Trace Views:
        \ui{Aggregated} (one row per ID, updated in-place) or
        \ui{Rolling Log} (chronological, append-only). \\
    Default timestamp
      & The timestamp column format used when a Trace View is first opened:
        \ui{Absolute} (local wall-clock time),
        \ui{Absolute (UTC)} (wall-clock in UTC),
        \ui{Relative} (seconds since measurement start), or
        \ui{Delta} (time since the previous frame of the same ID). \\
    Preferred save format
      & Pre-selects the output format in the \menu{Save Trace} dialog.
        Choices: Vector ASC, Vector MDF4, Linux candump, PCAP, PCAPng. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{lX}
    ...
    \textbf{Setting} & \textbf{Description} \\
    \midrule
    Decode UDS on 29-bit (extended) CAN IDs
      & When checked, the UDS/ISO-TP decoder also processes frames with
        29-bit extended IDs (in addition to the standard 11-bit IDs). \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{longtable}{p{4.5cm} p{8.5cm}}
  ...
  \key{Ctrl+Alt+S}       & Setup Interface \\
